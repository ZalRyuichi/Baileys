...
export const makeMessagesSocket = (config) => {
	    ...
	    await authState.keys.transaction(async () => {
		    ...
		    if (isGroupOrStatus &&!isRetryResend) {
	    		...
	    		for (const { user, jid } of devices) {
	    		    /** RX: opsi untuk skip sync pesan ke device lain milik akun sendiri (private chat) */
                    const isExactSenderDevice = jid === meId || (meLid && jid === meLid)
	    		...
	    		if (!isRetryResend && !isSelfChatDestination && otherRecipients.length === 0) {
	    			logger.warn({ jid, deviceCount: devices.length, RX }, 'relayMessage: no recipient devices resolved for the other party; aborting to avoid a silent no-op send')
    ...
    return {
        ...
        sendTable: async (jid, title, headers, rows, quoted, options = {}) => {
			...
			await relayMessage(jid, message, { messageId, RX: options.RX })
